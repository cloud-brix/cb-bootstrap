...
export EXEC_FILE="cluster-update-workers-mysql-cluster.sh"
...
# echo "."
# echo "."
# echo "--------$(hostname)/STARTING cluster-update-worker-mysql-cluster.sh"
# echo "--------$(hostname)/cluster-init-user.sh: whoami: $(whoami)"
# echo "--------$(hostname)/cluster-update-worker-mysql-cluster.sh: executing at the cluster member $clusterMember"
# echo "--------$(hostname)/cluster-update-worker-mysql-cluster.sh: setting up initial user for $clusterMember"
# echo "--------$(hostname)/cluster-update-worker-mysql-cluster.sh: check if cluster-init-user.sh is avilable"
# print header
cmdHead='
    source ${FX_DIR}
    fxHeader'
...
do
    ...
    # -------------------------------------------------------------------------------------------------------------------------------
    # echo "--------$(hostname)/cluster-update-worker-mysql-cluster.sh: pushing shared-files/pre-init-user.sh from $clusterMember to ${APP_NAME}$j"
    ...
    echo "--------$(hostname)/cluster-update-worker-mysql-cluster.sh: pushing shared-files/p from $clusterMember to ${APP_NAME}$j"
    # remove destination file
    lxc exec ${APP_NAME}$j -- rm -f /tmp/p
    # send file
    lxc file push /tmp/p ${APP_NAME}$j/tmp/p
    echo "--------$(hostname)/cluster-update-worker-mysql-cluster.sh: pushing worker-init-user.sh from $clusterMember to ${APP_NAME}$j"
    # remove destination file
    lxc exec ${APP_NAME}$j -- rm -f /tmp/worker-init-user.sh 
    # send file
    lxc file push /tmp/worker-init-user.sh      ${APP_NAME}$j/tmp/worker-init-user.sh
    echo "--------$(hostname)/cluster-update-worker-mysql-cluster.sh: setting up initial user at ${APP_NAME}$j"
    sudo lxc exec ${APP_NAME}$j -- sh /tmp/worker-init-user.sh
    echo "--------$(hostname)/cluster-update-worker-mysql-cluster.sh: pushing init_cluster.js from $clusterMember to ${APP_NAME}$j"
    ...
    echo "--------$(hostname)/cluster-update-worker-mysql-cluster.sh: pushing init_build_cluster.js from $clusterMember to ${APP_NAME}$j"
